...
class LogEntry {
  ...
  public String toString() {
    return String.format(
      "<LogEntry timestamp: %s  license: %s  location: %d  direction: %s  booth type: %s>",
      timestamp,
      licensePlate,
      location,
      direction,
...
public int countJourneys() {
  ...
  for(LogEntry e : logEntries) {
                  // System.out.println("e " + e.getBoothType());

    if(e.getBoothType().equalsIgnoreCase("ENTRY")) {
              // System.out.println("licenseplate " + count);

      counts.add(e.getLicensePlate());
    }
    if(e.getBoothType().equalsIgnoreCase("EXIT")) {
      if(counts.contains(e.getLicensePlate())) {
        count++;
        // System.out.println("count " + count);
        counts.remove(e.getLicensePlate());
      }
    }
  }
  return count;
